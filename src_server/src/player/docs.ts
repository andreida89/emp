...
type Document = 'passport' | 'licenses' | 'sindicat' | 'medcard' | 'faction';
...
class PlayerDocs {
	...
	offer(player: Player, type: Document) {
		...
		offers.showWithExpires(customer, player.mp.id, 'Arata documentele');
	}
	...
	private showPlayerDocs(player: Player, type: 'passport' | 'licenses' | 'sindicat', data: any) {
