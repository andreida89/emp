...
class Player {
    getName(player: PlayerMp, separator = false) {
        ...
        // Verifica daca playerul este admin
        const adminLvl = localPlayer.getVariable('adminLvl'); // Get the admin level of the viewer
        const id = player.remoteId;

        // If it's the local player, return their own name with formatting
        ...
        // If the current player is an admin, show full name + ID
        if (adminLvl) {
            return isFamiliar ? `${name.replace('_', ' ')} (${id})` : `Jucator (${id})`;
        }

        // If the current player is NOT an admin, always return "Cetatean (ID)"
        return ``;
    }
}
