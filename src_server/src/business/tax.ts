...
class BusinessTax {
	...
	getTax(data: Entity) {
		const { income } = data;
		return (income / 100) * this.taxPercent;
	}
	...
	async pay(player: Player, index: number, amount: any) {
		...
		if (!owning.isOwner(player, entity?.owner)) {
			return mp.events.reject('Nu sunteti proprietarul afacerii');
		}

		if (!isNumber(days) || days <= 0 || entity.paid + days > this.maxDays) {
			return mp.events.reject(
				`Casa poate fi platita doar pentru ${this.maxDays - entity.paid} zile`
			);
		}
