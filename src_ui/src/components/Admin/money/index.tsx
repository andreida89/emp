...
export default function AdminMoney() {
	async function giveMoney(player: string, amount: number) {
		...
		showNotification('success', 'Operatiune reusita');
	}
	...
	return (
		<div className="admin_money">
			...
			>
				{(formik) => (
					<Form>
						<Field className="admin_field" type="number" name="sum" placeholder="Suma" />

						<Players onChange={(data) => formik.setFieldValue('player', data.dbId)} />

						<GradientButton type="submit">Adauga</GradientButton>
					</Form>
				)}
			</Formik>
		</div>
