...
export default class DatabaseUser extends Component<Props> {
	...
	render() {
		...
		return (
			<Page>
				<Navbar title="Cetatean" backLink="Cautare" />

				<BlockHeader>Informatii principale</BlockHeader>
				<List inset>
				<ListItem
    title="Sosire in oras"
    after={moment(registrationAt).format('DD.MM.YYYY')}
/>
<ListItem title="Cont bancar" after={prettify.phoneNumber(bankAccount)} />
<ListItem title="Telefon" after={prettify.phoneNumber(phone)} />

				</List>

				<BlockHeader>Proprietati</BlockHeader>
				<List accordionList inset>
					<ListItem accordionItem title="Vehicul">
						<AccordionContent>
							<List>
				...
				<List inset>
					<ListItem link="#" title="Cautare" onClick={this.getWantedData.bind(this)} />
				</List>

				<List inset>
					<ListButton link="/fine/" title="Emite amenda" routeProps={{ userId }} />
					<ListButton
						link="/arrest/"
						title="Arestati"
						color="red"
