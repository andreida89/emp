...
class CuffsActions {
	...
	private useHandcuffs(player: Player) {
		...
		if (!handcuffs) {
			return hud.showNotification(player, 'error', 'Nu aveti catuse');
		}
	...
	private useCableTie(player: Player) {
		...
		if (!tie) {
			return hud.showNotification(player, 'error', 'Nu aveti coliere de plastic');
		}
	...
	private untiePlayer(player: Player) {
		...
		if (knife?.name !== 'knife') {
			return hud.showNotification(player, 'error', 'Luati cutitul in mana');
		}
