...
class CarTheft extends Job {
	constructor() {
		super(
			...
			{ name: 'Job Hot de Masini', model: 229, color: 1 }
		);
	...
	async startWork(player: Player, level: number) {
		if (player.level < 3) {
			return mp.events.reject('Necesar nivel 3 pentru a face acest job');
		}
		if (factions.isGovMember(player)) {
			return mp.events.reject('Nu au fost inca plasate comenzi noi, incearca mai tarziu');
		}
