...
class LSC extends Service {
	...
	constructor() {
		super('lscustoms', { name: 'Service Auto', model: 72, color: 4 }, 2);

		this.prices = {
			repair: [300],
			engine_sound: [3000],
			engine: [500, 1300, 10000, 30000, 50000],
	...
	private async buy(player: Player, product: Product, payment: PaymentType) {
		...
		await pay(player, payment, this.getPrice(vehicle, product), 'barbershop');
