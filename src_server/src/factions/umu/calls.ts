...
class UmuCalls {
	...
	constructor() {
		...
		mp.events.subscribe({
			'UmuCalls-GetList': this.getCallsList.bind(this),
			'UmuCalls-Accept': this.acceptCall.bind(this),
			'UmuCalls-Create': this.createCall.bind(this)
		});
	}
	...
	private getCallsList(player: Player) {
		if (!umu.inFaction(player)) throw new SilentError('access denied');
	...
	private createCall(player: Player) {
		...
		this.notifyMedics('Ai primit un nou apel, mergeti rapid!');
	}

	private acceptCall(player: Player, id: string) {
		const call = this.getCall(id);

		if (!call) return mp.events.reject('Acest apel a fost deja acceptat.');
		if (this.hasAcceptedCall(player)) return mp.events.reject('Ati acceptat deja apelul.');
		...
		if (victim) {
			...
			hud.showNotification(
				victim,
				'info',
				'Spitalul a acceptat apelul dumneavoastra, asteptati ambulanta.'
			);
		}
	}

	private notifyMedics(message: string) {
		umu.getPlayers(true).forEach((medic) => hud.showNotification(medic, 'info', message));
	}
}

export default new UmuCalls();
