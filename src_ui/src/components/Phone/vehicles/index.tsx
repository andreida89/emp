...
export default class Vehicles extends Component<{}, State> {
	...
	async getVehiclePosition() {
		...
		try {
			...
			showNotification('info', 'Locatia vehiculului a fost marcata pe harta');
		} catch (err: any) {
			showNotification('error', 'Trebuie sa faceti o cerere de spawn inainte');
		}		
	}
	...
	async spawnVehicle() {
		...
		try {
			...
			showNotification( 'info', 'Vehiculul dvs. va fi spawnat in curand' );
		} catch ( err: any ) {
...
async despawnVehicle() {
	...
	try {
		console.log('[DEBUG] despawnVehicle selectedVehicle.id:', selectedVehicle.id); // DEBUG AICI

		await rpc.callServer('Vehicle-DespawnItem', selectedVehicle.id);

		showNotification('info', 'Masina va fi parcata in cel mai scurt timp');
	} catch (err: any) {
		if (err.msg) showNotification('error', err.msg);
	}
}
			...
			<div className="vehicles">
				{selectedVehicle ? (
					...
					<div className="vehicles_main">
						<Title className="vehicles_main-title">Vehicolele dvs.</Title>
