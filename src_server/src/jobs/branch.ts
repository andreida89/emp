...
abstract class JobBranch {
	...
	protected async giveSalary(player: Player) {
		...
		await playerInventory.addItem(player, { name: 'ron', amount: this.salary });
		player.mp.call("AnuntNotification", [`Ai primit salariul de ${this.salary} RON (${this.name})`, 'success']);
	}
}
