...
class FactionWarehouse {
	...
	async changeMaterials(amount: number) {
		...
		if (totalAmount < 0) return mp.events.reject('Materiale insuficiente in depozit');
		if (totalAmount > this.capacity) return mp.events.reject('Depozitul este supraaglomerat');
