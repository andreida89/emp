...
class Voice {
	...
	private voiceRanges = [1, 8.0, 15.0]; // șoaptă, normal, strigă
	private currentLevelIndex = 1; // implicit: normal

	constructor() {
		this.listeners = [];
		this.range = this.voiceRanges[this.currentLevelIndex];
	...
	private cycleVoiceLevel() {
		this.currentLevelIndex = (this.currentLevelIndex + 1) % this.voiceRanges.length;
		this.range = this.voiceRanges[this.currentLevelIndex];

		hud.setVoiceLevel(this.currentLevelIndex + 1); // pentru UI React
		const name = ['Șoaptă', 'Normal', 'Strigă'][this.currentLevelIndex];
		mp.gui.chat.push(`Nivel voce: ${name}`);
	}

	addListener(player: PlayerMp, notify: boolean) {
		if (this.listeners.includes(player)) return;
		...
		setInterval(() => {
			...
			this.listeners.forEach((player) => {
				if (player.handle !== 0) {
					...
					else if (!this.autoVolume) player.voiceVolume = 1.0;
				} else {
