...
class Inventory {
	...
	// ---- AICI ADAUGI FUNCTIA NOUA ----
	private equipQuickSlot(index: number) {
		if (!mp.browsers.hud || player.isCuffed()) return;

		const slot = `quick_${index}`;
		const isExists = !!player.getVariable(slot);
...
for (let index = 1; index <= 4; index++) {
	...
	// leagă corect contextul cu arrow function!
	binder.bind(name, index.toString(), this.equipQuickSlot.bind(this, index));
}
	}
}
