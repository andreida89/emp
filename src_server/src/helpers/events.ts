...
class Events {
	...
	private setup() {
		// Assignments moved to module level
	}
}

global.SilentError = SilentError as typeof Error;

mp.events.subscribe = (events: any, authorized = true) => {
	Object.entries(events).forEach(([name, callback]) =>
		rpc.register(name, async (data: null | any[], info) => {
			const player = mp.players.get(info.player as PlayerMp);
			...
			try {
				...
				const result = isArray(data)
					? await (callback as any)(player, ...data)
					: await (callback as any)(player, data);
...
mp.events.subscribeToDefault = (events: any, authorized = true) => {
	Object.entries(events).forEach(([name, callback]) => {
		mp.events.add(name, async (...args: any[]) => {
			try {
				const [entity, ...data] = args;
...
mp.events.reject = (reason: any) => Promise.reject(new ClientError(reason));
