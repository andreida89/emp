...
class Offers {
	...
	private notifyAboutStatus(player: Player, status: boolean) {
		if (status) hud.showNotification(player, 'success', 'Propunerea dvs. a fost acceptata');
		else hud.showNotification(player, 'error', 'Propunerea dvs. a fost respinsa', true);
	}
}
