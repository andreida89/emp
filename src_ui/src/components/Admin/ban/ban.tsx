...
export default function Ban() {
	async function banPlayer( reason: string, term: string, player: string ) {
		try {
			...
			showNotification( 'success', 'Jucatorul a fost banat!' );
		} catch ( err: any ) {
		...
		>
			{( formik ) => (
				<Form>
					...
					<DatePicker name="term" placeholder="Perioada" />
					<Players onChange={( data ) => formik.setFieldValue( 'player', data.dbId )} />

					<Checkbox name="permanent" label="Permanent" />

					<GradientButton type="submit">Baneaza</GradientButton>
				</Form>
			)}
		</Formik>
	);
