...
export default class Referral extends Component<{}, State> {
	...
	async useCode( code: string ) {
		try {
			...
			showNotification( 'success', 'Cod activat cu succes' );
		} catch ( err: any ) {
		...
		return (
			<div className="referral">
				<Group className="referral_info">
					<Button current={code}>Cod</Button>
					<Button current={referrals.total.toString()}>Au introdus</Button>
					<Button current={referrals.confirmed.toString()}>Au primit bonus</Button>
				</Group>

				<Description>{`Dupa atingerea nivelului ${ confirmLevel }, jucatorul va primi ${ bonus }$, iar dvs. ${ income }$.`}</Description>
