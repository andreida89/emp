...
const Weapons: React.FC<Props> = (props) => {
  const [activeCategory, setActiveCategory] = useState('MELEE');
  ...
  const [locationStr, setLocationStr] = useState('STRADA PRINCIPALA');
  ...
  useEffect(() => {
    ...
    rpc.callServer('Supermarket-GetCash').then((cash: any) => {
  ...
  const categories = [
    ...
    { id: 'MELEE', label: 'ALBE', icon: <Icons.Sword size="0.9vw" /> },
    { id: 'FIREARM', label: 'FOC', icon: <Icons.Crosshair size="0.9vw" /> },
    { id: 'AMMO', label: 'AMUNITIE', icon: <Icons.Disc size="0.9vw" /> },
  ];

  const prices = (props.location?.state as any)?.prices || {};

  const products = useMemo(() => {
    const melee = ['bottle', 'dagger', 'bat', 'golfclub', 'knuckle', 'knife', 'flashlight'];
    const ammo = ['9mm', '7.62mm', '12gauge'];

    return Object.entries(prices).map(([name, price]) => {
      const itemData = (inventoryItems as any)[name] || {};
      let category = 'FIREARM';
      if (melee.includes(name)) category = 'MELEE';
      if (ammo.includes(name)) category = 'AMMO';
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(p => p.category === 'MELEE');
  }, [products]);
    ...
    setTimeout(async () => {
      try {
        ...
        for(const item of cart) {
          // Guns usually buy one by one in some systems, but we adapt Weapons-Buy to match Supermarket if needed
          // For now, call Weapons-Buy for each item.qty
          for(let i = 0; i < item.qty; i++) {
            const response = await rpc.callServer('Weapons-Buy', [item.id, paymentMethod]);
    ...
    <div className="magazin-nou-wrapper">
      ...
      <div className="header">
        <div className="header-title-box">
          <h1 className="header-title">GUNSHOP</h1>
          <p className="header-subtitle">WEAPONS STORE</p>
        </div>
            ...
            <div className="overlay">
              ...
              {checkoutStatus === 'SUCCESS' && (
                <div className="result-box">
                  ...
                  <h2 className="result-title success-text">TRANZACTIE REUSITA!</h2>
      ...
      <div className="footer">
          <div className="location-card">
            ...
            <span className="loc-val">{locationStr} GUNSHOP</span>
          </div>
      </div>
    </div>
  </div>
