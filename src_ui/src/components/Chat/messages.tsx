...
function prepareMessageText(text: string) {
	const msg = text
		.replace(/[<>]+/g, '') // elimină taguri HTML nedorite
		.replace(/!{([a-fA-F0-9]{6}|[a-fA-F0-9]{3})}/g, '<font style="color: #$1;">') // colorare text
		.replace(/\[badge(?::(\w+))?\](.*?)\[\/badge\]/g, '<span class="badge $1">$2</span>'); // badge parsing
