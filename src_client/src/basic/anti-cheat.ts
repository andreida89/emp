...
class AntiCheat {
	...
	init() {
		...
		setInterval(() => {
			...
			if (player.vehicle) {
				if (!this.isAllowedGroundDistOfVehicle(25)) this.report('FlyHack detectat.');
				if (this.checkVehicleMaxSpeed()) this.report('Speedhack detectat.');
			}
	...
	private checkVehicleMaxSpeed() {
		...
		return vehicle && !this.isAirVehicle(vehicle) && vehicle.getSpeed() > maxSpeed * 3;
	}

	private isAllowedGroundDistOfVehicle(limit = 80) {
