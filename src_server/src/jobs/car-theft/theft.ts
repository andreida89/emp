...
class Theft extends Branch {
	...
	startWork(player: Player) {
		if (this.workers.amount > 3) {
			hud.showNotification(player, 'error', 'Nu sunt inca indicii, vino mai tarziu!', true);
		...
		mp.blips.create(
			...
			{ model: 0, color: 48, name: 'Checkpoint' },
			player
		);
	}
	...
	private onUnlockVehicle(player: Player, vehicle: VehicleMp) {
		...
		hud.showNotification(
			player,
			'info',
			'Excelent! Livreaza masina la locul indicat pe harta.'
		);
	}
}
