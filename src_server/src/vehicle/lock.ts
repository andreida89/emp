...
class VehicleLock {
	...
	toggle(player: Player, vehicle: VehicleMp) {
		...
		if (!owning.isOwner(vehicle, player)) {
			return hud.showNotification(player, 'error', 'Nu ai cheia vehiculului');
		}
		...
		hud.showNotification(player, 'info', locked ? 'Ati deschis vehiculul' : 'Ati inchis vehiculul');
	}
	...
	private async open(player: Player, vehicle: VehicleMp) {
		...
		hud.showNotification(player, 'info', 'Ati descuiat vehiculul');
	}
