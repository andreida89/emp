...
buy = async () => {
	...
	await rpc.callServer('FactionWardrobe-BuyFreeUMU', data);
};



	async startWorkUMU() {
		await rpc.callServer('Factions-StartWorkUMU');
	...
	async finishWorkUMU() {
		...
		await rpc.callServer('Factions-FinishWorkUMU');
		...
		return (
			<div className="faction-wardrobe">
				...
				<div className="faction-wardrobe_buttons">
					{onDuty ? (
						<GradientButton color="purple" onClick={this.finishWorkUMU.bind(this)}>
							OFF DUTY
						</GradientButton>
					) : (
						<GradientButton onClick={this.startWorkUMU.bind(this)}>
							ON DUTY
						</GradientButton>
					)}
