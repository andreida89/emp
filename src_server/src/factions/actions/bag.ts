...
class BagActions {
	...
	private async useBag(player: Player) {
		...
		if (!bag) {
			return hud.showNotification(player, 'error', 'Pentru asta ai nevoie de un sac');
		}
	...
	private unsetBagOnHead(player: Player) {
		player.mp.setVariable('headsack', false);
		player.mp.setClothes(1, 0, 0, 0);
	}
}
