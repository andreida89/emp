...
export default function UserArrest( { f7router, userId }: Props ) {
	...
	return (
		<Page>
			<Navbar title="Arest" backLink="Inapoi" />
					...
					<Form>
						<List inset>
							<ListInput
								...
								type="number"
								placeholder="Durata"
								info="Specificati durata arestului (intre 1 si 100 de minute)"
								value={values.term}
							...
							<ListInput
								...
								type="text"
								placeholder="Motiv"
								info="Descrieti pe scurt motivul"
								value={values.reason}
								onChange={handleChange}
								onInputClear={handleChange}
							/>
						...
						<List inset>
							<ListButton title="Aresteaza" onClick={submitForm} />
						</List>
					</Form>
				)}
			</Formik>
