...
const Supermarket: React.FC<Props> = (props) => {
  ...
  const categories = [
    ...
    { id: 'BAUTURI', label: 'BAUTURI', icon: <Icons.Coffee size="0.9vw" /> },
    { id: 'DIVERSE', label: 'UTILITATI', icon: <Icons.Zap size="0.9vw" /> },
  ];
  ...
  const products = useMemo(() => {
    return Object.entries(prices).map(([name, price]) => {
      ...
      if (itemData?.type === 'food') category = 'ALIMENTE';
      if (itemData?.type === 'water' || itemData?.type === 'alcohol') category = 'BAUTURI';
    ...
    setTimeout(async () => {
      try {
        ...
        for(const item of cart) {
          const response = await rpc.callServer('Supermarket-Buy', [{ name: item.id, amount: item.qty }, paymentMethod]);
    ...
    <div className="magazin-nou-wrapper">
      ...
      <div className="header">
        <div className="header-title-box">
          <h1 className="header-title">MAGAZIN</h1>
          <p className="header-subtitle">Marketplace 24/7</p>
        </div>
      ...
      <div className="footer">
          <div className="location-card">
            ...
            <span className="loc-val">{locationStr} 24/7</span>
          </div>
      </div>
    </div>
  </div>
