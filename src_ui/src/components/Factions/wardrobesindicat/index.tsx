...
buy = async () => {
	...
	await rpc.callServer('FactionWardrobe-BuyFreeSindicat', data);
};



	async startWorkSindicat() {
		await rpc.callServer('Factions-StartWorkSindicat');
		this.setState(() => ({ onDuty: true }));
	}

	async finishWorkSindicat() {
		if (!this.state.onDuty) return;

		await rpc.callServer('Factions-FinishWorkSindicat');
