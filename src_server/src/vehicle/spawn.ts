...
class VehicleSpawn {
		...
		mp.players.withTimeout(
			...
			async () => {
				...
				hud.showNotification(
					player,
					'info',
					'Vehiculul a fost livrat, locatia este marcata pe GPS'
				);
			},
			3 * 1000
		);
	...
	private checkDeliveryErrors(player: Player, vehicle?: VehicleMp) {
		...
		if (player.mp.getOwnVariable('vehicleDelivery')) error = 'Vehiculul este deja pe drum';
		else if (passengers.isExists(vehicle)) error = 'A fost gasit un pasager in vehicul';
