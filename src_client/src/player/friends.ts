...
class PlayerFriends {
	...
	private prepareString(str: string) {
		...
		return str.replace(regexp, (match) => {
			...
			if (!mp.players.exists(player)) return ""; // If player doesn't exist, return nothing
	
			// Get the admin level of the person viewing the name
			const adminLvl = localPlayer.getVariable('adminLvl');
	
			// If the viewer is NOT an admin, always return "Cetatean (ID)"
			if (!adminLvl) return "";
	
			// Admins see real names, non-friends will still be "(ID)"
			return this.isFriend(player) ? `${player.name.replace('_', ' ')} (${id})` : "";
		});
	}
