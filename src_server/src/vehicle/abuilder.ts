...
const ARMORED_VEHICLES = [
	'kuruma2',
	'insurgent2',
	'nightshark'
];

class AVehicleBuilder {
	...
	private reset(model: string, position: Vector3Mp, heading: number, dimension: number) {
		...
		this.vehicle.setVariables({
			...
			state: { ...vehicleState.initial, locked: false }
		});

		// <<< INTEGRARE BLINDAT >>>
		if (ARMORED_VEHICLES.includes(model)) {
			this.vehicle.setVariable('isArmored', true);
		}
		// <<< END INTEGRARE BLINDAT >>>
	}
}

export default AVehicleBuilder;
